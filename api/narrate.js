...
export default async function handler(req,res){
 ...
 try{
  ...
  const out=JSON.parse(text);if(typeof out.narrative!=='string')throw new Error('narrative_missing');out.narrative=clean(out.narrative).slice(0,900);out.choices=Array.isArray(out.choices)?out.choices.map(choice=>clean(choice).slice(0,160)).filter(Boolean).slice(0,3):[];if(out.choices.length!==3)throw new Error('choices_missing');out.faction_updates=Array.isArray(out.faction_updates)?out.faction_updates.slice(0,4):[];out.npc_updates=Array.isArray(out.npc_updates)?out.npc_updates.slice(0,8):[];return res.status(200).json(out);
