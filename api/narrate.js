...
export default async function handler(req,res){
 ...
 try{
  const r=await fetch('https://ai-gateway.vercel.sh/v1/responses',{method:'POST',headers:{Authorization:`Bearer ${token}`,'Content-Type':'application/json'},body:JSON.stringify({model:MODEL,input:prompt,max_output_tokens:1200,text:{format:{type:'json_schema',name:'gm_turn',strict:true,schema:narrativeSchema}}})});
