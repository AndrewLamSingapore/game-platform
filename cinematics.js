...
function play(){
  if(!scene)return;playedThisPage.add(scene.key);clearTimeout(runTimer);clearInterval(progressTimer);prepare(scene);placeholder.hidden=true;vignette.hidden=false;generate.hidden=true;replay.hidden=true;progress.hidden=reduceMotion;progress.value=0;statusText.textContent='The world is remembering…';setBusy(true);
